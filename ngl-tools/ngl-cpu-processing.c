...
static struct ngl_node *get_scene(const char *filename)
{
    ...
    int framebuffer_dimensions[] = {256, 256};
    struct ngl_node *camera = ngl_node_create(NGL_NODE_CAMERA, render0);

    /* Set camera read callback */
    ngl_node_param_set(camera, "read_callback", read_callback);

    /* Set camera extra user data that will be passed to read callback, here
     * the uniform node that we want to modify */
    ngl_node_param_set(camera, "read_data", luminance);

    /* Set the rectangle that will be captured and passed to the read callback,
     * XXX: pipe_{width,height} param names does not make much sense with the
     * new read_callback param and will be renamed later on if the patch
     * reaches master */
    ngl_node_param_set(camera, "pipe_width", framebuffer_dimensions[0]);
    ngl_node_param_set(camera, "pipe_height", framebuffer_dimensions[1]);

    /* Use a render to texture to a dedicated framebuffer for capturing its output,
     * XXX: the color_texture is not used and should be replaced by a dedicated render buffer */
    struct ngl_node *color_texture = ngl_node_create(NGL_NODE_TEXTURE2D);
    ngl_node_param_set(color_texture, "width", framebuffer_dimensions[0]);
    ngl_node_param_set(color_texture, "height", framebuffer_dimensions[1]);
