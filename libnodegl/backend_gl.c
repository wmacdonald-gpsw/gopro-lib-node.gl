...
static int gl_offscreen_fbo_init(struct ngl_ctx *s)
{
...
static void gl_offscreen_fbo_reset(struct ngl_ctx *s)
{
...
static int gl_offscreen_capture_init(struct ngl_ctx *s)
{
    struct glcontext *gl = s->glcontext;
    struct ngl_config *config = &s->config;
    const int ios_capture = gl->platform == NGL_PLATFORM_IOS && config->window;

    s->capture = config->capture_buffer || ios_capture;
    if (!s->capture)
        return 0;
    ...
    if (gl->features & NGLI_FEATURE_FRAMEBUFFER_OBJECT) {
        ...
        if (config->samples > 0 && gl->backend == NGL_BACKEND_OPENGLES) {
            struct texture_params attachment_params = NGLI_TEXTURE_PARAM_DEFAULTS;
            attachment_params.format = NGLI_FORMAT_R8G8B8A8_UNORM;
            attachment_params.width = config->width;
            attachment_params.height = config->height;
...
static int gl_offscreen_capture(struct ngl_ctx *s)
{
    struct glcontext *gl = s->glcontext;
    struct ngl_config *config = &s->config;
    if (!s->capture)
    ...
    if (gl->features & NGLI_FEATURE_FRAMEBUFFER_OBJECT) {
        ...
        if (config->samples > 0 && gl->backend == NGL_BACKEND_OPENGLES) {
            struct fbo *resolve_fbo = &s->oes_resolve_fbo;
            ngli_fbo_blit(fbo, resolve_fbo, 0);
            ngli_fbo_bind(resolve_fbo);
            ngli_fbo_blit(resolve_fbo, capture_fbo, 1);
            ngli_fbo_unbind(resolve_fbo);
        } else {
...
static void gl_offscreen_capture_reset(struct ngl_ctx *s)
{
...
static int gl_post_draw(struct ngl_ctx *s, double t)
{
    ...
    int ret = gl_offscreen_capture(s);
    if (ret < 0)
