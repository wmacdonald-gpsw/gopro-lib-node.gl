...
static int update_uniforms(struct ngl_node *node)
{
    ...
    void *mapped_memory = ngli_graphic_buffer_map(vk, &s->uniform_buffer);
    ...
    ngli_graphic_buffer_unmap(vk, &s->uniform_buffer);
#else
    struct glcontext *gl = ctx->glcontext;
...
static VkResult create_descriptor_sets(struct ngl_node *node)
{
    ...
    if (nb_bindings) {
        ...
        VkDescriptorPoolSize *descriptor_pool_sizes = calloc(NGLI_ARRAY_NB(types), sizeof(struct VkDescriptorPoolSize));
...
int ngli_pipeline_init(struct ngl_node *node)
{
    ...
    if (uniform_buffer_size) {
        ...
        // allocate buffer
        int ret = ngli_graphic_buffer_allocate(vk,
                                               &s->uniform_buffer,
                                               uniform_buffer_size,
    ...
    if (uniform_buffer_size) {
        ...
        for (int i = 0; i < NGLI_ARRAY_NB(bindings_map); i++) {
            ...
            while ((block_entry = ngli_hmap_next(blocks, block_entry))) {
                ...
                if (binding->flag & NGLI_SHADER_UNIFORM) {
                    int aligned_size = NGLI_ALIGN(block->size, 32);
                    ngli_graphic_buffer_bind(vk, &s->uniform_buffer, s, uniform_block_offset, aligned_size, binding->index, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
                    ...
                    if (s->buffers) {
                        ...
                        if (ret < 0)
                            return ret;
                        ngli_graphic_buffer_upload(vk, graphic_buffer, buffer->data, buffer->data_size);
                        ngli_graphic_buffer_bind(vk, graphic_buffer, s, 0, buffer->data_size, binding->index, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
                    }
                }
            }
        }
...
#define GET_UNIFORM_VARIABLE(name) do {                                              \
    ...
    if (nb_buffers > 0 &&
        ...
        while ((entry = ngli_hmap_next(s->buffers, entry))) {
            ...
            int ret = ngli_graphic_buffer_allocate(gl, graphic_buffer, buffer->data_size, buffer->usage);
            if (ret < 0)
                return ret;
...
void ngli_pipeline_uninit(struct ngl_node *node)
{
    ...
    ngli_graphic_buffer_free(gl, &s->uniform_buffer);
#endif
}
